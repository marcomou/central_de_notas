...
@Component({
  selector: 'app-data-logistic-operator-subpage',
  templateUrl: './data-logistic-operator-subpage.component.html',
  styleUrls: ['./data-logistic-operator-subpage.component.scss'],
})
export class DataLogisticOperatorSubpageComponent implements OnInit {
  @Input()
      ...
      forkJoin([
        ...
        next: (response) => {
          ...
          this.organizationForm.patchValue({
            ...
            legal_type_id:
              this.ecoMembership?.member_organization?.legal_type?.id,
          });
  ...
  get legalTypeName() {
    const legalTypeId = this.organizationForm.get('legal_type_id')?.value;

    console.log();
    if (legalTypeId) {
      return this.legalTypes?.find(({ id }) => id === legalTypeId)?.name || '';
    }
    return '';
  }
  ...
  // TODO: Add state list
  get stateName() {
    const state = this.logisticOperator.get('extra')?.get('state')?.value;
