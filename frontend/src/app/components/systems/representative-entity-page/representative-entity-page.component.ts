...
@Component({
  selector: 'app-representative-entity-page',
  templateUrl: './representative-entity-page.component.html',
  styleUrls: ['./representative-entity-page.component.scss'],
})
export class RepresentativeEntityPageComponent implements OnInit, OnDestroy {
  public representativeEntityForm: FormGroup;
  ...
  constructor(
    ...
    this.representativeEntityForm = this.formBuilder.group({
      id: [],
      member_role: ['liable'],
      eco_duty_id: [],
    ...
    this.organizationForm = this.formBuilder.group({
      ...
      legal_name: [''],
      front_name: [''],
      legal_type_id: [{ value: '', disabled: this.hasEcoMembership() }],
    });

    if (this.router.getCurrentNavigation()?.extras.state?.['ecoMembership']) {
      const ecoMembershipId =
        this.router.getCurrentNavigation()?.extras.state?.['ecoMembership']?.id;
      ...
      forkJoin([
        ...
        next: (response) => {
          ...
          this.representativeEntityForm.patchValue({
  ...
  get canUpdateRepresentativeEntity() {
    return this.ruleService.systemRules.canUpdateRepresentativeEntity;
  }
  ...
  get stateName() {
    return this.representativeEntityForm.get('extra')?.get('state')?.value || '';
  }

  get representantives(): FormArray {
    return this.representativeEntityForm.controls[
      'representantives'
    ] as FormArray;
  }

  get interloctors(): FormArray {
    return this.representativeEntityForm.controls['interloctors'] as FormArray;
  }
      ...
      .subscribe({
        next: (response) => {
          ...
          this.ecoMembershipService
            .storeEcoMemberShip(this.representativeEntityForm.value)
            .subscribe({
  ...
  public update = () => {
    const requests = [
      ...
      this.ecoMembershipService.updateEcoMembership(
        this.representativeEntityForm.value
      ),
    ];

    this.interloctors.value.map((interloctor: any) => {
      interloctor.eco_membership_id = this.representativeEntityForm.value?.id;
      if (interloctor.id) {
